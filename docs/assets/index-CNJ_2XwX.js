...
Please report this to https://github.com/markedjs/marked.`,e){let e=`<p>An error occurred:</p><pre>`+I(n.message+``,!0)+`</pre>`;return t?Promise.resolve(e):e}if(t)return Promise.reject(n);throw n}}};function G(e,t){return W.parse(e,t)}G.options=G.setOptions=function(e){return W.setOptions(e),G.defaults=W.defaults,a(G.defaults),G},G.getDefaults=r,G.defaults=i,G.use=function(...e){return W.use(...e),G.defaults=W.defaults,a(G.defaults),G},G.walkTokens=function(e,t){return W.walkTokens(e,t)},G.parseInline=W.parseInline,G.Parser=H,G.parser=H.parse,G.Renderer=B,G.TextRenderer=V,G.Lexer=z,G.lexer=z.lex,G.Tokenizer=R,G.Hooks=U,G.parse=G,G.options,G.setOptions,G.use,G.walkTokens,G.parseInline,H.parse,z.lex,G.setOptions({gfm:!0,breaks:!0});async function He(e,t){if(t){Ke(e,t);return}e.innerHTML=`
  ...
  `;let n=document.getElementById(`featured-releases`),r=document.getElementById(`older-releases-list`),i=document.querySelectorAll(`.filter-btn`);try{let e=await(await fetch(`https://api.github.com/repos/OriolCS2/CometEngine/releases`)).json();e.sort((e,t)=>new Date(t.published_at)-new Date(e.published_at));let t=e.find(e=>!e.prerelease),a=e.find(e=>e.prerelease&&(!t||new Date(e.published_at)>new Date(t.published_at)));n.innerHTML=``,t&&n.appendChild(Ge(t,`Latest Stable Release`)),a&&n.appendChild(Ge(a,`Latest Release Candidate`));let o=e=>{r.innerHTML=e.map(e=>`
      ...
      `).join(``)||`<div style="padding: 2rem; color: var(--text-dim);">No releases found.</div>`,r.querySelectorAll(`.older-release-item`).forEach(e=>{e.onmouseenter=()=>{e.style.backgroundColor=`rgba(255, 140, 0, 0.1)`},e.onmouseleave=()=>{e.style.backgroundColor=`transparent`}})};o(e),i.forEach(t=>{t.addEventListener(`click`,()=>{i.forEach(e=>e.classList.remove(`active`)),t.classList.add(`active`);let n=t.getAttribute(`data-filter`),r=e;n===`stable`&&(r=e.filter(e=>!e.prerelease)),n===`rc`&&(r=e.filter(e=>e.prerelease)),o(r)})})}catch(e){console.error(`Error fetching releases:`,e),n.innerHTML=`<p>Error loading releases.</p>`}}function Ue(e){if(!e)return`No release notes provided.`;try{return G.parse(e.trim())}catch(t){return console.error(`Markdown parsing error:`,t),e.replace(/\n/g,`<br>`)}}function We(e){if(!e)return`No release notes provided.`;let t=e.trim();t=t.replace(/^#{1,6}[^\n]*(\n|\r\n)*/,``),t=t.trim();try{return G.parse(t)}catch{return t.replace(/\n/g,`<br>`)}}function Ge(e,t){let n=document.createElement(`div`);n.className=`release-card`,n.style.display=`flex`,n.style.flexDirection=`column`;let r=We(e.body);return n.innerHTML=`
    <style>
  ...
  `,n}async function Ke(e,t){e.innerHTML=`<div class="container" style="padding: 100px 2rem;"><div class="loading">Loading release ${t}...</div></div>`;try{let n=await(await fetch(`https://api.github.com/repos/OriolCS2/CometEngine/releases/tags/${t}`)).json(),r=n.assets||[],i=r.some(e=>e.name.toLowerCase().includes(`windows`)),a=r.some(e=>e.name.toLowerCase().includes(`linux`)),o=r.some(e=>e.name.toLowerCase().includes(`mac`)),s=r.some(e=>![`windows`,`linux`,`mac`].some(t=>e.name.toLowerCase().includes(t))),c=qe(),l=`windows`;c===`linux`&&a?l=`linux`:c===`mac`&&o?l=`mac`:i?l=`windows`:a?l=`linux`:o?l=`mac`:s&&(l=`other`),e.innerHTML=`
    ...
    `;let u=document.getElementById(`assets-list`),d=document.querySelectorAll(`#platform-tabs .filter-btn`),f=e=>{let t=[];t=e===`other`?r.filter(e=>![`windows`,`linux`,`mac`].some(t=>e.name.toLowerCase().includes(t))):r.filter(t=>t.name.toLowerCase().includes(e)),u.innerHTML=t.map(e=>`
      ...
      `).join(``)||`<div style="padding: 2rem; color: var(--text-dim); grid-column: 1/-1;">No downloads found for ${e}.</div>`};f(l),d.forEach(e=>{e.addEventListener(`click`,()=>{d.forEach(e=>e.classList.remove(`active`)),e.classList.add(`active`),f(e.getAttribute(`data-platform`))})})}catch(t){e.innerHTML=`<div class="container" style="padding: 100px 2rem;"><h2>Error loading release details</h2><p>${t.message}</p></div>`}}function qe(){let e=window.navigator.platform.toLowerCase();return e.includes(`win`)?`windows`:e.includes(`linux`)?`linux`:e.includes(`mac`)?`mac`:`windows`}var K=null,Je=[],q=new Set,J=``;async function Ye(e,t){let n=!K;n&&(e.innerHTML=`<div class="loading">Parsing documentation...</div>`,K=await Ze(),Je=nt(K));let r=decodeURIComponent(t.replace(`#docs`,``).substring(1));if(r){let e=r.split(`::`),t=``;e.forEach(e=>{t=t?`${t}::${e}`:e,q.add(t)})}if(n||!document.getElementById(`docs-tree`))e.innerHTML=`
    ...
    `;let u=o.querySelector(`.tree-toggle`),d=o.querySelector(`.tree-label`),f=o.querySelector(`.tree-children`),p=u.querySelector(`i`);return u.addEventListener(`click`,e=>{if(e.stopPropagation(),!c)return;let t=f.style.display===`block`;f.style.display=t?`none`:`block`,p&&(p.className=`fas ${t?`fa-chevron-right`:`fa-chevron-down`}`),t?q.delete(s):q.add(s)}),d.addEventListener(`click`,e=>{e.stopPropagation(),window.location.hash=`#docs/${s}`}),Object.keys(t).filter(e=>e!==`_members`).sort().forEach(e=>{f.appendChild(i(e,t[e],s))}),o};Object.keys(t).sort().forEach(n=>{e.appendChild(i(n,t[n]))})}function $e(e){let t=e.split(`::`),n=K;for(let e of t)n=n?.[e];if(!n)return`<h2>Element not found</h2>`;let r=n._members||[],i=r.find(e=>e.type===`T`),a=r.filter(e=>e.isConstructor),o=r.filter(e=>e.isCallback),s=r.filter(e=>e.type===`F`),c=r.filter(e=>e.type===`M`&&!e.isConstructor&&!e.isCallback),l=r.filter(e=>e.type===`P`),u=Object.keys(n).filter(e=>e!==`_members`).map(e=>({name:e,summary:n[e]._members?.find(e=>e.type===`T`)?.summary||``}));return`
  ...
  `}function Z(e){if(!e)return``;let t=e.replace(/&lt;/g,`<`).replace(/&gt;/g,`>`),n=t.match(/^array<(.+)>$/);if(n){let e=n[1];return`<span style="color:#61afef;">array&lt;${Z(e)}&gt;</span>`}let r=t.replace(/[?*&]/g,``).trim(),i=Je.find(e=>e===r||e.endsWith(`::${r}`));return i?`<a href="#docs/${i}" style="color:#61afef;text-decoration:underline;">${t}</a>`:`<span style="color:#61afef;">${t}</span>`}function nt(e,t=``){let n=[];return Object.keys(e).forEach(r=>{if(r===`_members`)return;let i=t?`${t}::${r}`:r;n.push(i),n=n.concat(nt(e[r],i))}),n}function rt(e,t){let n={},r=(e,n)=>{let i=!1;if(e._members){let r=e._members.filter(e=>e.name.toLowerCase().includes(t)||e.summary.toLowerCase().includes(t));r.length>0&&(n._members=r,i=!0)}return Object.keys(e).forEach(a=>{if(a===`_members`)return;let o={},s=r(e[a],o),c=a.toLowerCase().includes(t);if((s||c)&&(n[a]=o,i=!0,c&&e[a]._members&&(!o._members||!o._members.find(e=>e.type===`T`)))){let t=e[a]._members.find(e=>e.type===`T`);t&&(o._members||=[],o._members.unshift(t))}}),i};return r(e,n),n}function it(){return`
  ...
  `}var Q=document.getElementById(`app`),at=document.querySelectorAll(`.nav-links a`);function $(){let t=window.location.hash||`#home`;at.forEach(e=>{e.getAttribute(`href`)===t?e.classList.add(`active`):e.classList.remove(`active`)}),t===`#home`?e(Q):t.startsWith(`#releases`)?He(Q,t.replace(`#releases`,``).substring(1)):t.startsWith(`#docs`)&&Ye(Q,t)}window.addEventListener(`hashchange`,$),window.addEventListener(`load`,$),document.addEventListener(`click`,e=>{e.target.tagName===`A`&&e.target.getAttribute(`href`)?.startsWith(`#docs/`)});