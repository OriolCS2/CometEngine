...
Please report this to https://github.com/markedjs/marked.`,e){let e=`<p>An error occurred:</p><pre>`+I(n.message+``,!0)+`</pre>`;return t?Promise.resolve(e):e}if(t)return Promise.reject(n);throw n}}};function G(e,t){return W.parse(e,t)}G.options=G.setOptions=function(e){return W.setOptions(e),G.defaults=W.defaults,o(G.defaults),G},G.getDefaults=i,G.defaults=a,G.use=function(...e){return W.use(...e),G.defaults=W.defaults,o(G.defaults),G},G.walkTokens=function(e,t){return W.walkTokens(e,t)},G.parseInline=W.parseInline,G.Parser=H,G.parser=H.parse,G.Renderer=B,G.TextRenderer=V,G.Lexer=z,G.lexer=z.lex,G.Tokenizer=R,G.Hooks=U,G.parse=G,G.options,G.setOptions,G.use,G.walkTokens,G.parseInline,H.parse,z.lex,G.setOptions({gfm:!0,breaks:!0});async function Ue(e,t){if(t){qe(e,t);return}e.innerHTML=`
  ...
  `,t.appendChild(n),document.body.appendChild(t),setTimeout(()=>{t.style.opacity=`1`,n.style.transform=`scale(1)`},10),t.onclick=()=>{t.style.opacity=`0`,n.style.transform=`scale(0.9)`,setTimeout(()=>t.remove(),300)}};async function Ze(e,t){let n=!K;n&&(e.innerHTML=`<div class="loading">Parsing documentation...</div>`,K=await $e(),Xe=rt(K));let r=decodeURIComponent(t.replace(`#docs`,``).substring(1));if(r){let e=r.split(`::`),t=``;e.forEach(e=>{t=t?`${t}::${e}`:e,q.add(t)})}if(n||!document.getElementById(`docs-tree`))e.innerHTML=`
  ...
  `}var $=document.getElementById(`app`),ot=document.querySelectorAll(`.nav-links a`);function st(){let e=window.location.hash||`#home`;ot.forEach(t=>{t.getAttribute(`href`)===e?t.classList.add(`active`):t.classList.remove(`active`)}),e===`#home`?t($):e.startsWith(`#releases`)?Ue($,e.replace(`#releases`,``).substring(1)):e.startsWith(`#docs`)&&Ze($,e)}window.addEventListener(`hashchange`,st),window.addEventListener(`load`,st),document.addEventListener(`click`,e=>{e.target.tagName===`A`&&e.target.getAttribute(`href`)?.startsWith(`#docs/`)});